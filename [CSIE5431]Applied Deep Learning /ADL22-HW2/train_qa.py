...
def parse_args():
    ...
    parser.add_argument(
        "--train_file", type=str, default="./processed_data/qa/train.json", help="A csv or a json file containing the training data."
    )
    parser.add_argument(
        "--preprocessing_num_workers", type=int, default=1, help="A csv or a json file containing the training data."
    )
    parser.add_argument(
        "--validation_file", type=str, default="./processed_data/qa/valid.json", help="A csv or a json file containing the validation data."
    )
    parser.add_argument(
        "--max_seq_length",
        type=int,
...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.pretrained_model_name_or_path, use_fast=True)

    if args.pretrained_model_name_or_path:
        model = AutoModelForQuestionAnswering.from_pretrained(
            args.pretrained_model_name_or_path,
            from_tf=bool(".ckpt" in args.pretrained_model_name_or_path),
            config=config,
        )
    else:
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        logger.info(f"Acc: {acc / len(prediction.predictions):.3f}")
